...
参与者:游客、注册用户(继承游客)、管理员(继承注册用户);外部系统:Redis GEO、天地图。完整图见 `use-case.mmd` / `use-case.puml`,分组与关系见 `03-用例图.md`。核心关系:创建/编辑 `<<include>>` 地图选点;附近查询 `<<extend>>` 列表浏览;审核 `<<include>>` 查看详情。
