...
| 主流程 | 1. 用户填写名称、类别、描述、图片 URL；2. 通过用例 PICK 在地图上选点取得 (lng, lat)；3. 系统校验字段；4. 写入 Redis Hash（status=PENDING/APPROVED）并在审核通过时 `GEOADD geo:landmarks lng lat lid`，反向索引集合同管道维护；5. 返回创建成功 |
| 替代流程 | 3a. 校验失败 → 返回 422 与字段级错误；4a. Redis 写入失败 → 整管道不提交，返回 500 |
